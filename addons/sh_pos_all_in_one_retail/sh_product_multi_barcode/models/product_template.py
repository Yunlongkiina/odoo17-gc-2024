...
class ShProductTemplate(models.Model):
    ...
    barcode_ids = fields.One2many(
        related='product_variant_ids.barcode_ids', readonly=False, ondelete="cascade")
    ...
    def check_uniqe_name(self):
        for rec in self:
            if self.env.company and self.env.company.sh_multi_barcode_unique:
                multi_barcode_id = self.env['product.barcode.multi'].search([('name', '=', rec.barcode)])
                if multi_barcode_id:
    ...
    def create(self, vals_list):
        ...
        for template, vals in zip(templates, vals_list):
            ...
            if vals.get('barcode_ids'):
                related_vals['barcode_ids'] = vals['barcode_ids']
            if related_vals:
